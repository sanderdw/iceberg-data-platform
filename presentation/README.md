...
`scripts/presentation-screenshots.mjs` signs in through Keycloak, seeds the demo story (three teams of a fictional grid operator, five databases, four users with a role per team), runs the five example notebooks as `sander`, shares one of the resulting tables with a municipality as team administrator `mila` and writes every PNG in `screenshots/`. It needs both stacks running and the Playwright Chromium (`npx playwright install chromium`).
