...
def main():
    ...
    try:
        with stack_resources() as (p, teams, databases, users, suffix):
            ...
            account = p.create_user(UserInput(name=f"analyst-{suffix}", memberships=[{"team": t, "role": "writer"} for t in teams[:2]]))
            users.append(account["user"]["id"])
            other = p.create_user(UserInput(name=f"other-{suffix}", memberships=[{"team": teams[2], "role": "reader"}]))
            users.append(other["user"]["id"])
            teammate = p.create_user(UserInput(name=f"alex-{suffix}", memberships=[{"team": teams[0], "role": "reader"}]))
