...
Catalog tools mirror the portal's read operations: `list_databases` (your teams, roles and databases), `list_namespaces`, `list_tables`, `describe_table`, `describe_view` and `preview_rows` (up to 100 rendered rows). They use your own Polaris grants, do not run SQL and cannot change catalog contents. `list_databases` includes databases whose deletion can be resumed, and under `sharedDatabases` the databases other teams share with any of your teams: these are read-only, the catalog tools show only their shared tables and views, and they cannot be renamed or deleted. Other tools take its opaque database ID because display names repeat across environments.
