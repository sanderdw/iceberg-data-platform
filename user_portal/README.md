...
Choose **Reports → New report**, or **Create report** from a catalog table. Select
the table, filters, aggregation and grouping, then a table, KPI, bar, line, area or
scatter visualization. **Run report** queries the latest committed Iceberg snapshot.
Choose UTC or Europe/Amsterdam for timestamp grouping. **View SQL** shows generated
SQL; **Edit as SQL copy** preserves the visual report and opens a separate SQL draft.
SQL uses the selected table as `source`, named `$parameters` and a reviewed SELECT
subset with aggregates and CTEs. File/URL functions and queries against other tables
are unavailable. SQL parameter values are entered as a JSON object.

Save reports for the active team and environment. Readers can create and copy
definitions without write access to Iceberg; the author or a team administrator can
edit/delete them. Concurrent saves return a conflict. Select **New dashboard**, add
saved reports, order cards, choose half/full widths and optionally map one shared
equality filter to each card's source column. Save before running a dashboard.
Each run uses the referenced reports' latest saved revisions. A referenced report
must be removed from its dashboards before it can be deleted.

Every viewer queries with their own permissions. A denied card shows an error while
authorized cards can finish. Definitions contain SQL and filter values and are visible
to all team members in that environment. The result cache is private to the viewing
session and lasts at most 60 seconds; a new snapshot misses it. **Refresh now** bypasses
it. **Cancel run** stops the worker. Switching workspace or signing out discards results.
Charts are static SVGs generated by dbt Charts; portal filters rerun the query. Result
tables preserve large integers, decimals and nanosecond timestamps as text where needed;
chart numeric projections can be approximate. No reporting tables or pipelines are created.

Reports support one source table, at most 20 output columns and 1,000 rows. Dashboards
have up to eight cards; a scope holds up to 200 definitions. Jobs are limited to two
per session and four globally, with a 45-second limit per card. Narrow large queries
with filters and aggregation. Native SQL has an explicit function allowlist.

`npm run users:build` builds the reporting image alongside the gateway and notebooks.
The image can be overridden through `REPORTING_IMAGE`. Compose stores definitions and
their revisions in `/data/reports.sqlite` on the `reports-data` volume, separate from
notebook files. Outside Compose, `USER_REPORTS_PATH` defaults to `.data/reports.sqlite`.
This store supports one gateway instance. Back up the whole reports volume while the
gateway is stopped, including SQLite WAL/SHM files if present; restore it before starting
the gateway. Ordinary `docker compose down` preserves it; do not use `--volumes` when
retaining saved reports. Sessions, jobs and cached data are transient across restarts.

## Tests

The [reporting runtime documentation](reporting/README.md) describes the separately
locked Python 3.13 image, isolation boundaries and live API smoke test.
