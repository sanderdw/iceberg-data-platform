...
The Iceberg v3 examples cover the v3 types and capabilities that DuckDB 1.5.5 writes natively. `GEOGRAPHY` and `UNKNOWN` are planned for DuckDB 2.0, and DuckDB has no nanosecond timestamp with time zone (`timestamptz_ns`), so the examples leave those out. The portal's table preview and the starter notebook read tables with PyIceberg, which cannot read `variant`, `geometry` or `geography` columns yet: **Preview** explains this for such tables and the starter notebook shows a notice. Use example 5 or your own DuckDB cells for those tables. Other v3 tables, including `timestamp_ns` columns and default values, preview normally.
