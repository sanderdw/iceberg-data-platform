...
function loginScreen() { state = null; database = null; namespace = []; browseVersion++; resetEditor(); $('#workspace-screen').hidden = true; $('#identity').hidden = true; $('#login-screen').hidden = false; clearNotice(); placeNotice(); }
...
function renderState() {
  ...
  $('#databases').replaceChildren(...state.databases.map(d => { const b = element('button', undefined, `db${database === d.id ? ' selected' : ''}`); const label = element('span', d.name); label.append(element('small', 'DATABASE')); b.append(symbol('database'), label); b.setAttribute('aria-pressed', String(database === d.id)); b.addEventListener('click', () => browse(d.id, [])); return b; }));
  if (!state.databases.length) $('#databases').append(element('p', 'This team has no databases yet.', 'hint'));
  $('#notebooks').replaceChildren(...state.notebooks.map(n => { const b = element('button', `${n.database} ↗`, 'quiet'); b.addEventListener('click', () => showNotebook(n)); return b; }));
  if (!state.notebooks.length) $('#notebooks').append(element('p', 'Open a database in marimo to get started.', 'hint'));
}
function clearBrowser() { clearNotice(); database = null; namespace = []; browseVersion++; $('#database-label').textContent = 'CATALOG'; $('#namespace-title').textContent = 'Select a database'; $('#breadcrumbs').replaceChildren(); $('#open-notebook').hidden = true; $('#examples').hidden = true; $('#objects').replaceChildren(element('p', 'Select a database from the catalog.', 'empty')); }
async function loadState() {
  const next = await api('/workspace');
  if (state && (state.activeTeam !== next.activeTeam || (database && !next.databases.some(d => d.id === database)))) { resetEditor(); clearBrowser(); }
...
async function browse(db, ns) {
  clearNotice(); const version = ++browseVersion; database = db; namespace = [...ns]; $('#editor').hidden = true; $('#browser').hidden = false; renderState();
  ...
  $('#breadcrumbs').replaceChildren(...crumbs.flatMap((parts, i) => { const b = element('button', parts.at(-1) || db); b.addEventListener('click', () => browse(db, parts)); return i ? [element('span', '/'), b] : [b]; }));
  ...
  try {
    ...
    const result = await api(`/contents?${query}`); if (version !== browseVersion) return;
    const rows = [];
    ...
    result.tables.forEach(t => row(t.name, 'Iceberg table', 'table', 'marimo ↗', () => openNotebook(db, ns, t.name)));
    result.views.forEach(v => row(v.name, 'Iceberg view', 'view', 'marimo ↗', () => openNotebook(db, ns)));
    $('#objects').replaceChildren(...rows); if (!rows.length) $('#objects').append(element('p', 'This namespace is empty. Continue working in marimo with your user permissions.', 'empty'));
  } catch (error) { if (version === browseVersion) { $('#objects').replaceChildren(element('p', error.message, 'empty')); notice(error.message, true); } }
}
function showNotebook(notebook, targetUrl = notebook.url) { if (activeNotebook?.id !== notebook.id || activeNotebook?.selectedUrl !== targetUrl) { const frame = element('iframe'); frame.title = `marimo · ${notebook.database}`; frame.src = targetUrl; frame.allow = 'clipboard-read; clipboard-write'; $('#frame-host').replaceChildren(frame); } activeNotebook = {...notebook, selectedUrl: targetUrl}; $('#notebook-file').replaceChildren(...[{title: 'My workspace', url: notebook.url}, ...(notebook.examples || [])].map(n => { const option = element('option', n.title); option.value = n.url; return option; })); $('#notebook-file').value = targetUrl; $('#editor-title').textContent = notebook.database; $('#editor-external').href = targetUrl; $('#browser').hidden = true; $('#editor').hidden = false; placeNotice(); }
async function openNotebook(db, ns, table, exampleIndex) { const existing = state.notebooks.find(n => n.database === db); if (existing) { showNotebook(existing, exampleIndex === undefined ? existing.url : existing.examples?.[exampleIndex]?.url); if (exampleIndex === undefined) notice('Your existing notebook for this database has resumed. Select the table you want to work with there.'); return; } notice('Starting your personal marimo workspace…'); const notebook = await api('/notebooks', 'POST', {database: db, namespace: ns, table: table || null}); await loadState(); showNotebook(notebook, exampleIndex === undefined ? notebook.url : notebook.examples?.[exampleIndex]?.url); notice('Marimo is ready. Changes are saved in your personal workspace.'); }
...
$('#refresh').addEventListener('click', e => busy(e.currentTarget, async () => { await loadState(); if (database) await browse(database, namespace); }));
$('#open-notebook').addEventListener('click', e => busy(e.currentTarget, () => openNotebook(database, namespace)));
