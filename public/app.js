...
function renderTable() {
  ...
  for (const b of document.querySelectorAll('[data-role]')) b.onclick = () => roleModal(b.dataset.role);
...
const roleSelect = () => `<label>Access<select name="role">${Object.entries(roleNames).map(([value, label]) => `<option value="${value}">${esc(label)}</option>`).join('')}</select></label>`;
function roleModal(id) {
  const user = state.data.users.find(u => u.id === id);
  openModal('Edit role', `Change access for ${esc(user.name)}.`, `<form id="modal-form">${roleSelect()}<p class="subtle">The role applies to every database in this user’s teams. Existing database credentials stay the same. Direct S3 access is available only with Database + bucket administration.</p><p class="form-error" role="alert"></p><div class="modal-actions"><button class="button primary" type="submit">Save</button></div></form>`);
  $('#modal-form [name=role]').value = user.role;
  formSubmit(async input => {
    const result = await api(`/users/${encodeURIComponent(id)}/role`, 'PATCH', input);
    closeModal(); notifyStatus('User role and permissions updated.');
...
function userModal(database) {
  ...
  openModal('A new user', 'Create a service account and assign it to one or more teams.', `<form id="modal-form">${nameInput('Username', 'e.g. pipeline_reader')}${teamChecks(selected ? [selected] : [])}${roleSelect()}<div class="info-box">${icon('key')}<span>This role applies to all current and future databases in the selected teams. Secrets are shown only once.</span></div><p class="form-error" role="alert"></p><div class="modal-actions"><button class="button primary" type="submit">Create user ${icon('arrow')}</button></div></form>`);
  formSubmit(async input => {
