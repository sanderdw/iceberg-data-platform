...
function formSubmit(handler) {
  $('#modal-form').onsubmit = async event => {
    ...
    try { const data = new FormData(form), input = Object.fromEntries(data); if (form.querySelector('[name=teams]')) { input.memberships = data.getAll('teams').map(team => ({ team, role: data.get(`role:${team}`) })); for (const key of Object.keys(input)) if (key === 'teams' || key.startsWith('role:')) delete input[key]; } await handler(input); }
