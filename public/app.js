...
const roleSelect = () => `<label>Access<select name="role">${Object.entries(roleNames).filter(([value]) => !keycloakUsers || value !== 'bucket-admin').map(([value, label]) => `<option value="${value}">${esc(label)}</option>`).join('')}</select></label>`;
function roleModal(id) {
  ...
  if (keycloakUsers) $('#modal-form .subtle').textContent = 'The role applies to databases in this user’s teams. Database administration does not grant access to this administration portal.';
