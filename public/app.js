...
const envNames = { development: 'Development', staging: 'Staging', production: 'Production' };
...
function render() {
  ...
  const titles = { databases: 'Databases', users: 'Users', teams: 'Teams', explorer: 'Catalog', infrastructure: 'Infrastructure', guide: 'Getting started' };
  $('#app').innerHTML = `<div class="shell"><aside class="sidebar"><a class="brand" href="/"> <img src="/favicon.svg" alt="">iceberg<span> / </span></a><div class="workspace"><span class="workspace-icon">L</span><div>Local workspace<small>DevOps platform</small></div></div><nav>${[['databases', 'database', 'Databases'], ['users', 'users', 'Users'], ['teams', 'layers', 'Teams'], ['explorer', 'book', 'Catalog'], ['infrastructure', 'layers', 'Infrastructure']].map(([id, i, label]) => `<button data-page="${id}" class="nav-item ${state.page === id ? 'active' : ''}">${icon(i)}${label}${id === 'databases' ? `<span class="nav-count">${databases.length}</span>` : ''}</button>`).join('')}</nav><div class="sidebar-bottom"><div class="local-note"><span class="dot ${online ? '' : 'offline'}"></span><strong>${online ? 'Your stack is connected' : 'Stack unavailable'}</strong><p>Local development environment</p></div><button class="nav-item ${state.page === 'guide' ? 'active' : ''}" data-page="guide">${icon('book')}Getting started ${icon('arrow')}</button><button class="theme-button" data-theme-toggle aria-label="Toggle color theme"></button><button class="profile" id="logout" aria-label="Sign out"><span class="avatar">PA</span><span>Platform admin<small>Local administrator</small></span>${icon('logout')}</button></div></aside><div class="main"><header class="topbar"><div>Workspace <span>/</span> <strong>${titles[state.page]}</strong></div><span class="local-badge"><span class="dot"></span> Local development</span><button class="icon-button mobile-logout" id="logout-mobile" aria-label="Sign out">${icon('logout')}</button></header><main class="content"><div class="page-heading"><div><div class="eyebrow">WORKSPACE / ADMINISTRATION</div><h1>${titles[state.page]}<span>.</span></h1><p>${({ databases: 'Your databases. Each with its own bucket and permissions.', users: 'Users and access through one or more teams.', teams: 'Create teams and manage their databases and members.', explorer: 'All databases, namespaces, tables and views. For portal administrators only.', infrastructure: 'An open foundation for all your databases.', guide: 'From your team to your first database.' })[state.page]}</p></div>${['databases', 'users', 'teams'].includes(state.page) ? `<button class="button primary" id="create" ${!online || (state.page !== 'teams' && !teams.length) ? 'disabled' : ''}>${icon('plus')}${({ databases: 'Create database', users: 'Create user', teams: 'Create team' })[state.page]}</button>` : ''}</div>${!online ? '<div class="alert" role="alert">The data provider is unavailable. Start the stack with <code>docker compose up -d</code> and refresh the page.</div>' : ''}<div id="status" class="inline-status" role="status" aria-live="polite">${state.status ? `[ ${esc(state.status)} ]` : ''}</div><section class="stats"><div><span>Databases ${icon('database')}</span><strong>${online ? databases.length.toString().padStart(2, '0') : '—'}</strong><small>CATALOGS + BUCKETS</small></div><div><span>Users ${icon('users')}</span><strong>${online ? users.length.toString().padStart(2, '0') : '—'}</strong><small>SERVICE ACCOUNTS</small></div><div><span>Teams ${icon('layers')}</span><strong>${online ? teams.length.toString().padStart(2, '0') : '—'}</strong><small>OWNERS</small></div><div class="status-stat"><span>Platform status <span class="dot ${online ? '' : 'offline'}"></span></span><strong class="status-text">${online ? 'Connected' : 'Offline'}</strong><small>${esc(health.provider)} available</small></div></section><div id="page-content"></div><footer><span><span class="dot"></span> ICEBERG / CONTROL PLANE</span><span>Iceberg Data Platform <span class="footer-divider">/</span> Local workspace</span></footer></main></div></div>`;
  for (const nav of document.querySelectorAll('[data-page]')) nav.onclick = () => { state.page = nav.dataset.page; state.query = ''; render(); };
  $('#logout').onclick = async () => { try { await api('/session', 'DELETE'); login(); } catch (e) { notifyStatus(e.message); } };
...
const teamChecks = selected => `<fieldset class="team-choices"><legend>Teams · select at least one</legend>${state.data.teams.map(t => `<label><input type="checkbox" name="teams" value="${esc(t.id)}" ${selected.includes(t.id) ? 'checked' : ''}>${esc(t.name)}</label>`).join('')}</fieldset>`;
...
async function renderExplorer() {
  ...
  try {
    ...
    const nodes = result.databases.map(database => {
      ...
      const node = branch(database.id, [], database.name, `${team} · ${database.managed ? 'Managed in portal' : 'Created outside portal'}`);
...
function renderTable() {
  ...
  const items = (isDb ? databases : users).filter(item => `${item.name} ${isDb ? teamName(item.team) : item.teams.map(teamName).join(' ')}`.toLowerCase().includes(state.query.toLowerCase()) && (!isDb || ((!state.team || item.team === state.team) && (!state.environment || item.environment === state.environment))));
  if (!items.length) {
  ...
  for (const b of document.querySelectorAll('[data-memberships]')) b.onclick = () => membershipsModal(b.dataset.memberships);
...
function formSubmit(handler) {
  $('#modal-form').onsubmit = async event => {
    ...
    try { const data = new FormData(form), input = Object.fromEntries(data); if (form.querySelector('[name=teams]')) input.teams = data.getAll('teams'); await handler(input); }
...
const nameInput = (label, placeholder) => `<label>${label}<input name="name" required minlength="3" maxlength="48" pattern="[a-z][a-z0-9_\\-]{2,47}" placeholder="${placeholder}" autocomplete="off"><small>3–48 characters: lowercase letters, digits, - or _</small></label>`;
...
function databaseModal() {
  openModal('A new database', 'A dedicated place for your team data.', `<form id="modal-form">${nameInput('Database name', 'e.g. analytics_dev')}<div class="form-row"><label>Team<select name="team" required>${teamOptions()}</select></label><label>Environment<select name="environment">${Object.entries(envNames).map(([v, l]) => `<option value="${v}">${l}</option>`).join('')}</select></label></div><label>Description <span class="optional">optional</span><textarea name="description" maxlength="280" placeholder="What will your team use this database for?"></textarea></label><div class="info-box">${icon('layers')}<span>Your database gets its own bucket. Existing team members automatically receive access with their selected role.</span></div><p class="form-error" role="alert"></p><div class="modal-actions"><button class="button primary" type="submit">${icon('plus')}Create database</button></div></form>`);
...
function moveModal(id) {
  ...
  openModal('Move database', `${esc(db.name)} will be owned by another team.`, `<form id="modal-form"><label>Team<select name="team" required>${teamOptions(db.team)}</select></label><div class="info-box">${icon('users')}<span>Members of the new team receive access. Members of the previous team lose access unless they also belong to the new team. The bucket and connection details stay the same.</span></div><p class="form-error" role="alert"></p><div class="modal-actions"><button class="button primary" type="submit">Move</button></div></form>`);
...
function membershipsModal(id) {
  ...
  openModal('Edit teams', `Manage memberships for ${esc(user.name)}.`, `<form id="modal-form">${teamChecks(user.teams)}<p class="subtle">Database and bucket permissions will match these teams.</p><p class="form-error" role="alert"></p><div class="modal-actions"><button class="button primary" type="submit">Save</button></div></form>`);
  formSubmit(async input => { if (!input.teams.length) throw new Error('Select at least one team.'); await api(`/users/${encodeURIComponent(id)}`, 'PATCH', input); closeModal(); notifyStatus('Team memberships updated.'); await load(); });
}
function userModal(database) {
  const selected = state.data.databases.find(d => d.id === database)?.team;
  openModal('A new user', 'Create a service account and assign it to one or more teams.', `<form id="modal-form">${nameInput('Username', 'e.g. pipeline_reader')}${teamChecks(selected ? [selected] : [])}<label>Access<select name="role"><option value="reader">Read — view data</option><option value="writer">Read & write — manage data and tables</option><option value="admin">Administrator — manage data, metadata and access</option><option value="bucket-admin">Database + bucket administration — full access to team buckets</option></select></label><div class="info-box">${icon('key')}<span>This role applies to all current and future databases in the selected teams. Secrets are shown only once.</span></div><p class="form-error" role="alert"></p><div class="modal-actions"><button class="button primary" type="submit">Create user ${icon('arrow')}</button></div></form>`);
  formSubmit(async input => {
    if (!input.teams.length) throw new Error('Select at least one team.');
    const result = await api('/users', 'POST', input);
    openModal('Your user is ready', 'Save these credentials now. You cannot view the secret again later.', `<div class="credential"><span>Client-ID</span><code>${esc(result.credentials.clientId)}</code></div><div class="credential"><span>Client-secret</span><code>${esc(result.credentials.clientSecret)}</code></div>${result.bucketCredentials ? `<div class="code-label">S3 · your team buckets</div><div class="credential"><span>S3 access key</span><code>${esc(result.bucketCredentials.accessKeyId)}</code></div><div class="credential"><span>S3 secret key</span><code>${esc(result.bucketCredentials.secretAccessKey)}</code></div><p class="subtle">Use these S3 credentials at ${esc(result.bucketCredentials.endpoint)} or in the RustFS console.</p>` : ''}<div class="info-box">${icon('check')}<span>${esc(input.name)} belongs to ${input.teams.map(t => esc(teamName(t))).join(', ')} with role ${esc(roleNames[input.role])}.</span></div><div class="modal-actions"><button class="button" id="copy-credentials">Copy credentials</button><button class="button primary" id="saved">Saved securely</button></div>`);
...
async function confirmAction(title, description, path, status) {
  openModal(title, description, '<form id="modal-form"><p class="form-error" role="alert"></p><div class="modal-actions"><button class="button" type="button" id="cancel-delete">Cancel</button><button class="button destructive" type="submit">Delete</button></div></form>');
