...
test('central teams, memberships, move and deletion through FastAPI', async ({ page, context }) => {
  ...
  try {
    ...
    const access = team => page.getByRole('combobox', { name: `Access for ${team}`, exact: true });
    await expect(access(renamed)).toHaveValue('reader');
    await access(second).selectOption('writer');
    await page.getByRole('dialog').getByRole('button', { name: 'Create user' }).click();
    ...
    await expect(userRow).toContainText(`${renamed} · Read`);
    await expect(userRow).toContainText(`${second} · Read & write`);
    const editAccess = () => userRow.getByRole('button', { name: 'Edit access', exact: true }).click();
    const save = () => page.getByRole('dialog').getByRole('button', { name: 'Save', exact: true }).click();
    await editAccess();
    await expect(access(renamed)).toHaveValue('reader');
    await expect(access(second)).toHaveValue('writer');
    await access(renamed).selectOption('admin');
    await page.screenshot({ path: 'test-results/edit-user-access.png', fullPage: true });
    await save();
    await expect(page.getByRole('dialog')).not.toBeVisible();
    await expect(userRow).toContainText(`${renamed} · Administrator`);
    await expect(userRow).toContainText(`${second} · Read & write`);
    await editAccess();
    await access(renamed).selectOption('bucket-admin');
    await save();
    ...
    for (const role of ['reader', 'bucket-admin']) {
      await editAccess();
      await access(renamed).selectOption(role);
      await save();
      await expect(page.getByRole('dialog')).not.toBeVisible();
    }
    await expect(userRow).toContainText(`${renamed} · Database + bucket administration`);
    await editAccess();
    await page.getByLabel(renamed, { exact: true }).uncheck();
    await expect(access(renamed)).toBeDisabled();
    await save();
    await expect(page.getByRole('dialog')).not.toBeVisible();
