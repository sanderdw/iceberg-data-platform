...
- **MCP clients** use the public `iceberg-mcp` client: PKCE, no secret, and a single redirect to `http://localhost:<MCP_CALLBACK_PORT>/callback`. Both portals accept only tokens with `azp = iceberg-mcp` on `/mcp`, so portal tokens are refused there, and they authorize every call. The admin endpoint also needs `platform-admin`, which comes from the client's `roles` default scope. Keep token exchange disabled.
...
- MCP clients keep an offline refresh token on the user's machine. It is valid for 30 days of inactivity. Revoke it in Keycloak under the user's offline **Sessions**.
